  ...
  return (
    <div className="mx-auto my-10 w-[350px] space-y-6 rounded-xl bg-white px-4 pb-8 pt-4 font-sans shadow-lg dark:bg-[#18181B]">
      ...
      <div className="flex items-center justify-between gap-6 text-sm md:text-base">
          ...
          <button className="t w-[80%] rounded-full py-2 font-medium text-gray-400 shadow-[0px_0px_10px_#E2DADA] duration-500  hover:scale-95 hover:bg-[#0095FF] hover:text-white hover:shadow-xl dark:shadow-[0px_2px_8px_0px_rgba(0,0,0,0.8)]"><Link to={`books/${_id}`}>View Details </Link></button>
      
        <Link to={`edit/${_id}`}><FaRegEdit className="w-7 h-7 text-green-600 hover:text-green-800 " /></Link>
        <button onClick={() => handleDeleteReviews(_id)}><RiDeleteBinLine className="w-7 h-7 text-rose-500 hover:text-rose-800" /></button>
      </div>
    </div>
  )
}
